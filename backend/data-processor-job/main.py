...
def extract_text_from_file(bucket_name, file_name):
    ...
    # For PDFs, return placeholder (will enhance later)
    if file_name.endswith('.pdf'):
        return f"[PDF content from {file_name}]"
    
    # For images, return placeholder (will add Gemini Vision later)
...
def main():
    ...
    try:
        ...
        text_content = extract_text_from_file(bucket, file_name)
        print(f"Extracted text ({len(text_content)} chars): {text_content[:200]}...")
